...
**name:** job-scraper
**description:** Scrapes Norwegian job sites (finn.no, jobbnorge.no, nav.arbeidplassen.no) for new positions matching your profile. Deduplicates across runs. Triggers on: job scrape, find jobs, search jobs, new jobs, job search, scrape jobs, /scrape
...
This skill searches multiple Norwegian job sites (finn.no, jobbnorge.no, nav.arbeidplassen.no) using targeted queries based on your profile, deduplicates against previously seen jobs and the application tracker, and presents new matches with a quick fit assessment.
...
For each search:
- Use `WebSearch` with site-specific queries (finn.no, jobbnorge.no, nav.arbeidplassen.no, linkedin.com/jobs)
- Target your configured geographic area (e.g. Oslo, Bergen, Trondheim, Stavanger)
...
- Use `WebFetch` to retrieve the job posting page
  - For finn.no: fetch the job listing URL directly
  - For jobbnorge.no: fetch the job detail page
  - For nav.arbeidplassen.no: fetch the job announcement page
